...
namespace ApplicationIntegration
{
	// Called before the program initializes
	static void InitBegin()
	{
		Profiler::GetInstance().InitStart();
		//ProfilerExample::DoNothing();
	}
	// Called after the program initializes
	static void InitEnd()
	{
		Profiler::GetInstance().InitEnd();
	}

	// Called at teh beginning of the program exit procedure
	static void ExitBegin()
	{
	...
	// Called before the main loop starts (not called every frame)
	static void PreMainLoop()
	{
		Profiler::GetInstance().Start();
	}
	// Called after the main loop exits (not called every frame)
	static void PostMainLoop()
	{
		Profiler::GetInstance().Stop();
	}

	// Called at the beginning of every frame
	static void FrameBegin(int32 frameCount)
	{
		//UNUSED(frameCount);
		Profiler::GetInstance().FrameBegin(frameCount);
	}
	// Called at the end of every frame
	static void FrameEnd(int32 frameCount)
	{
		//UNUSED(frameCount);
		Profiler::GetInstance().FrameEnd(frameCount);
	}
	...
	{
		//UNUSED(key);
		Profiler::GetInstance().KeyPressed(key);
	}
}
...
namespace TestMain
{
	...
	void EnableHeadlessMode(int framesToRun = 100, int testToRun = 4) {
	...
	{
		_frameCounter++;
		ApplicationIntegration::FrameBegin(_frameCounter);
		...
		ApplicationIntegration::FrameEnd(_frameCounter);
	}
	...
	{
		ApplicationIntegration::KeyPressed(key);
...
{
	ApplicationIntegration::InitBegin();
	...
	ApplicationIntegration::InitEnd();

	ApplicationIntegration::PreMainLoop();
	...
	ApplicationIntegration::PostMainLoop();

	ApplicationIntegration::ExitBegin();
	...
	ApplicationIntegration::ExitEnd();
