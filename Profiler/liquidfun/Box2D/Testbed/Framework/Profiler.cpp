...
//
//    if (addr < 0x100000 || addr > 0x7FFFFFFFFFFF)
//        return;
//
//    auto& sample = samples_[addr];
//    if (!sample.SymbolName_.empty()) {
//        if (sample.SymbolName_.find("std::") == 0 || sample.SymbolName_.find("__") == 0)
//            return;
//    }
//
//    auto now = std::chrono::steady_clock::now();
//    entryTimes_[addr].push_back(now);
//    sample.HitCount_++;
//    sample.Addr_ = addr;
//
//    if (sample.SymbolName_.empty()) {
...
//        symbol->SizeOfStruct = sizeof(SYMBOL_INFO);
//        symbol->MaxNameLen = MAX_SYM_NAME;
//        if (SymFromAddr(GetCurrentProcess(), addr, nullptr, symbol)) {
//            sample.SymbolName_ = symbol->Name;
//        }
//    }
//}
